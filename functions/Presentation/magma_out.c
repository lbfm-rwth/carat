...
/*--------------------------------------------------------------------------
  Prozedur zur Ausgabe einer Praesentation im magma-Format.
  Parameter:
      presentation - Praesentation
      filename - Name des outputfiles
---------------------------------------------------------------------------*/
void magma_out (presentation,filename)
...
fprintf(outfile," > := FreeGroup(%d); \n", presentation->generators->firstfree);
fprintf(outfile, " rels := { ");
...
fprintf(outfile," G := quo<F|rels>;\n");
fprintf(outfile," Order(G);\n");
fclose(outfile);
free(gens);
} 
